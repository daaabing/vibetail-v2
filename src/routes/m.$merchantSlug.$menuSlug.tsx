...
function MenuLanding() {
  ...
  const theme = getMenuTheme(menu.menuTheme);
  ...
  if (!ageOk) {
    return (
      <div
        className="w-full md:max-w-md md:mx-auto min-h-svh flex flex-col items-center justify-center px-6 text-center relative"
        style={theme ? { background: theme.landingBackground } : undefined}
      >
        {theme?.landingDecoration}
        <div className="relative z-10 flex flex-col items-center">
          {Placeholder ? <Placeholder size={110} /> : <VibetailLogo size={110} />}
          <h1
          ...
          <div className="mt-8 flex gap-3">
            <button
              ...
              style={theme ? { ...theme.ctaStyle, fontFamily: "var(--font-heading)" } : {
  ...
  return (
    <div
      className="w-full md:max-w-2xl lg:max-w-3xl md:mx-auto min-h-svh flex flex-col items-center justify-center px-6 py-10 text-center relative"
      style={theme ? { background: theme.landingBackground } : undefined}
    >
      {theme?.landingDecoration}
      <div className="relative z-10 flex flex-col items-center">
        {Placeholder ? <Placeholder size={140} /> : <VibetailLogo size={140} />}
        <div
          className="mt-4 text-[10px] uppercase tracking-[0.3em]"
          style={{ color: "var(--app-text-muted)", fontFamily: "var(--font-body)" }}
        >
          {theme?.eyebrow?.en ?? `Vibetail × ${menu.merchantName}`}
        </div>
        <h1
