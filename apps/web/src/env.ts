...
const serverEnvSchema = z
  .object({
    ...
    IMAGE_CUTOUT_PROVIDER: z.enum(["original", "sam2", "openrouter", "replicate", "replicate-sam2"]).default("original"),
  ...
  .superRefine((env, context) => {
    ...
    if (env.IMAGE_CUTOUT_PROVIDER === "openrouter") {
      requireFields(env, ["OPENROUTER_API_KEY"], context);
    }
    if (env.IMAGE_CUTOUT_PROVIDER === "replicate" || env.IMAGE_CUTOUT_PROVIDER === "replicate-sam2") {
