...
const NO_AUTH = { provider: "none", supabaseUrl: null, supabasePublishableKey: null, googleEnabled: false } as const;
...
describe("venue HTTP slice (local supabase)", () => {
  ...
  it("serves the publishable auth config and no secrets", async () => {
    ...
    expect(body).toEqual({
      auth: { provider: "none", supabaseUrl: null, supabasePublishableKey: null, googleEnabled: false },
    });
  });
