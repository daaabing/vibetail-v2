...
export function VenueExperience({ client, initialPreferences, initialResult, menu }: VenueExperienceProps) {
  ...
  if (noVisibleItems || noActiveItems) return <UnavailableMenu menu={menu} locale={locale} onLocale={() => setLocale((v) => v === "en" ? "zh" : "en")} />;
  ...
  return <LangContext.Provider value={locale}><div className="vt-page"><SiteHeader /><main className="vt-match-main">
    <MatchFlow
      context={{
        ...
        title: locale === "zh" ? "从这张菜单找到今晚这一杯" : "Match your vibe from this menu",
...
function UnavailableMenu({ locale, menu, onLocale }: { locale: Lang; menu: VenueMenu; onLocale(): void }) {
