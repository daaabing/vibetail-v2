...
import type { RestaurantClient, RestaurantMatchResult, RestaurantMenu, RestaurantPreferences } from "@vibetail/contracts";
...
export function RestaurantExperience({ client, initialPreferences, initialResult, menu }: RestaurantExperienceProps) {
  const [locale, setLocale] = useState<"en" | "zh">(initialPreferences?.locale ?? "en");
  useSeo(`${menu.restaurant.name} · ${menu.name} — Vibetail`, menu.shortIntro ?? menu.restaurant.shortIntro ?? "Match your mood to this live menu.");
  ...
  return <div className="vt-page"><SiteHeader /><main className="vt-narrow">
    <MatchFlow
      context={{
        kicker: `${menu.restaurant.name} · ${menu.name}`,
      ...
      locale={locale}
      match={(preferences) => client.matchItem(menu.restaurant.slug, menu.slug, preferences)}
    />
  </main><SiteFooter /></div>;
}

function UnavailableMenu({ locale, menu, onLocale }: { locale: "en" | "zh"; menu: RestaurantMenu; onLocale(): void }) {
  const empty = menu.items.length === 0;
  return <div className="vt-page"><SiteHeader /><main className="vt-narrow"><header className="vt-page-title vt-match-title"><div><p className="vt-kicker">{menu.restaurant.name} · {menu.name}</p><button className="vt-locale-toggle" type="button" onClick={onLocale}>{locale === "en" ? "中文" : "EN"}</button></div><h1>{empty ? (locale === "zh" ? "菜单还是空的" : "This menu is empty") : (locale === "zh" ? "目前没有可点项目" : "Nothing is available right now")}</h1><p>{locale === "zh" ? "请稍后再来，或向酒吧询问今晚的菜单。" : "Please check back later or ask the bar about tonight’s menu."}</p></header></main><SiteFooter /></div>;
}
