import { restaurantMatchResultSchema, restaurantPreferencesSchema, type RestaurantMatchResult, type RestaurantPreferences } from "@vibetail/contracts";
...
export function saveMatchHandoff(path: string, preferences: RestaurantPreferences, result: RestaurantMatchResult): void {
...
export function readMatchHandoff(path: string, now = Date.now()): { preferences: RestaurantPreferences; result: RestaurantMatchResult } | undefined {
  try {
...
export function parseMatchHandoff(value: unknown, path: string, now: number): { preferences: RestaurantPreferences; result: RestaurantMatchResult } | undefined {
  ...
  const preferences = restaurantPreferencesSchema.safeParse(candidate.preferences);
  const result = restaurantMatchResultSchema.safeParse(candidate.result);
  if (!preferences.success || !result.success) return undefined;
  const expectedPath = `/m/${result.data.restaurant.slug}/${result.data.menu.slug}`;
