...
`RESTAURANT_REPOSITORY=supabase` requires `SUPABASE_URL` and `SUPABASE_PUBLISHABLE_KEY` for public reads. `SUPABASE_SERVICE_ROLE_KEY` is optional and enables the temporary legacy management boundary; without it, management fails closed with `503`. `MODEL_PROVIDER=openai` requires server-only `MODEL_API_KEY` and `MODEL_NAME` (recommended starting point: `gpt-5.6-terra`). Runtime validation fails at startup when selected-provider configuration is incomplete. Service-role and model secrets are imported only by the server composition tree and are never exposed to client code.
