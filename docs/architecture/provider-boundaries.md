...
Remote model providers may include Vertex AI, Gemini, OpenAI, or Alibaba models. `deterministic` is the credential-free test/demo fallback. Image generation is intentionally absent; a future `ImageProvider` would be a separate product decision and interface.
...
| remote `MODEL_PROVIDER` | `MODEL_API_KEY`, `MODEL_NAME` | server/worker only |
...
- Shared provider contract tests will be implemented once executable providers arrive; Phase 1 makes no live integration claim.
