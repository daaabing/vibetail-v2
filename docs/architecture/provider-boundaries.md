...
The XPRIZE remote path is Vertex AI Express Mode through the Google Gen AI SDK, implemented as a distinct `vertex` adapter for the bounded Tasting Agent. It requires strict JSON output, a request timeout, sanitized usage/response metadata, and the same post-provider allowlist revalidation as every other model. Its credentialed canary and public Railway verification remain release gates and must not be inferred from mocked tests. OpenRouter Chat Completions and direct OpenAI Responses remain explicit alternative adapters; there is no silent production fallback. Direct Gemini Developer API and Alibaba remain future adapters behind the same interface. `deterministic` is the credential-free local/test fallback. Image generation remains a separate product decision and interface.
