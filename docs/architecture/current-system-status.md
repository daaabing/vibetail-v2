...
当前 deterministic matcher 是规则评分器，不是大模型。它会从允许的候选 item 中选择一个 ID 并生成模板化解释。系统也不会让 AI 自由生成酒名、价格、配料或不存在的 menu item。
...
- deployment pipeline 和 staging environment；
- Railway/Alibaba production resources；
