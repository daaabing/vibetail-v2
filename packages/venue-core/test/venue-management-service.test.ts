...
const TAPROOM_ID = "99999999-9999-4999-8999-999999999999";
const SIGNATURE_MENU_ID = "99999999-0001-4001-8001-000000000011";
const SMOKED_PEAR_ID = "77777777-0001-4001-8001-000000000001";
const ESPRESSO_ID = "77777777-0003-4003-8003-000000000003";

function createService(repository = new FixtureVenueRepository()) {
...
describe("venue sessions", () => {
  it("logs in by account name, keeps the session, and revokes it on logout", async () => {
    ...
    const login = await service.login("The Test Bar");
    expect(login.session.account.name).toBe("the test bar");
    expect(login.session.venue).toBeNull();
    const session = await service.getSession(login.token);
    expect(session.account.displayName).toBe("The Test Bar");
...
describe("venue creation", () => {
  it("creates a venue with a unique slug and blocks a second venue", async () => {
    ...
    const { token } = await service.login("Second Taproom");
    const session = await service.createVenue(token, {
      name: "Vibetail Taproom",
      address: "1 Test Street",
      venueType: "cocktail_bar",
    });
    expect(session.venue?.slug).toBe("vibetail-taproom-2");
    ...
    const created = repository.fixture.merchants.find((entry) => entry.slug === "vibetail-taproom-2");
...
describe("drink library", () => {
  ...
  it("propagates drink edits to every published menu that references it", async () => {
    ...
    const smokedPear = drinks.find((drink) => drink.id === SMOKED_PEAR_ID);
    expect(smokedPear).toBeDefined();
    await service.updateDrink(token, SMOKED_PEAR_ID, {
      ...drinkInput,
      name: "Smoked Pear 2.0",
      strength: "zero",
    });
    const lookup = await repository.lookupMenu("vibetail-taproom", "signature");
    expect(lookup.kind).toBe("ok");
    if (lookup.kind !== "ok") return;
    const item = lookup.menu.items.find((entry) => entry.id === SMOKED_PEAR_ID);
  ...
  it("reports usage and removes a deleted drink from all menus", async () => {
    const { service, repository } = createService();
    const token = await demoToken(service);
    const usage = await service.getDrinkUsage(token, ESPRESSO_ID);
    expect(usage.menus.map((menu) => menu.name).sort()).toEqual(["Signature Menu", "Winter Lab"]);
    const result = await service.deleteDrink(token, ESPRESSO_ID);
    expect(result.removedFromMenus).toBe(2);
    const drinks = await service.listDrinks(token);
    expect(drinks.some((drink) => drink.id === ESPRESSO_ID)).toBe(false);
    const lookup = await repository.lookupMenu("vibetail-taproom", "signature");
    if (lookup.kind !== "ok") throw new Error("expected published menu");
    expect(lookup.menu.items.some((item) => item.id === ESPRESSO_ID)).toBe(false);
  });
...
describe("venue menus", () => {
  it("creates a menu from library drinks and rejects unknown drinks", async () => {
    const { service } = createService();
    const token = await demoToken(service);
    const menu = await service.createMenu(token, { name: "Autumn Menu", drinkIds: [SMOKED_PEAR_ID] });
    expect(menu.status).toBe("draft");
    expect(menu.drinkIds).toEqual([SMOKED_PEAR_ID]);
    await expect(service.createMenu(token, {
      name: "Bad Menu",
  ...
  it("publishing auto-archives the previously published menu", async () => {
    ...
    const token = await demoToken(service);
    const menu = await service.createMenu(token, { name: "Autumn Menu", drinkIds: [SMOKED_PEAR_ID] });
    const menus = await service.publishMenu(token, menu.id);
    const byId = new Map(menus.map((entry) => [entry.id, entry.status]));
    expect(byId.get(menu.id)).toBe("published");
    expect(byId.get(SIGNATURE_MENU_ID)).toBe("archived");
    const lookup = await repository.lookupMenu("vibetail-taproom", "signature");
    expect(lookup.kind).toBe("menu_unpublished");
    const current = await repository.lookupMenu("vibetail-taproom", "autumn-menu");
  ...
  it("refuses to publish a menu without drinks", async () => {
    const { service } = createService();
    const token = await demoToken(service);
    const menu = await service.createMenu(token, { name: "Empty Lab", drinkIds: [] });
    await expect(service.publishMenu(token, menu.id)).rejects.toMatchObject({
  ...
  it("deleting a menu keeps its drinks in the library", async () => {
    const { service } = createService();
    const token = await demoToken(service);
    const before = (await service.listDrinks(token)).length;
    const menu = await service.createMenu(token, { name: "Disposable", drinkIds: [SMOKED_PEAR_ID] });
    await service.deleteMenu(token, menu.id);
    expect((await service.listDrinks(token)).length).toBe(before);
    const menus = await service.listMenus(token);
  ...
  it("supports renaming and replacing drinks without changing the slug", async () => {
    const { service } = createService();
    const token = await demoToken(service);
    const updated = await service.updateMenu(token, SIGNATURE_MENU_ID, {
      name: "Signature vNext",
      drinkIds: [SMOKED_PEAR_ID],
    });
    expect(updated.name).toBe("Signature vNext");
    expect(updated.slug).toBe("signature");
    expect(updated.drinkIds).toEqual([SMOKED_PEAR_ID]);
  });
});
...
describe("match recording and feedback", () => {
  function matchResult(): VenueMatchResult {
    return {
      venue: {
        id: TAPROOM_ID,
        slug: "vibetail-taproom",
        name: "Vibetail Taproom",
        shortIntro: null,
        logoUrl: null,
        coverImageUrl: null,
      },
      menu: { id: SIGNATURE_MENU_ID, slug: "signature", name: "Signature Menu" },
      item: {
        id: SMOKED_PEAR_ID,
        menuId: SIGNATURE_MENU_ID,
  ...
  it("records a match, accepts one feedback, and rejects duplicates", async () => {
    const { service } = createService();
    const matchId = await service.recordMatch(matchResult());
    ...
    const token = await demoToken(service);
    const stats = await service.getDashboard(token, "today");
  ...
  it("records menu views only for known venues", async () => {
    const { service, repository } = createService();
    await service.recordMenuView({ merchantSlug: "vibetail-taproom" });
    await service.recordMenuView({ merchantSlug: "nope-does-not-exist" });
    const count = await repository.countMenuViews(TAPROOM_ID, new Date(Date.now() - 60_000).toISOString());
    expect(count).toBe(1);
  });
});
...
describe("QR codes", () => {
  it("encodes the stable venue URL", async () => {
    const { service } = createService();
    const token = await demoToken(service);
    const qr = await service.getQr(token);
    expect(qr.consumerUrl).toBe("http://127.0.0.1:3000/m/vibetail-taproom");
    expect(qr.qrSvg).toContain("http://127.0.0.1:3000/m/vibetail-taproom");
  });

  it("requires a venue before issuing a QR code", async () => {
    const { service } = createService();
    const { token } = await service.login("No Venue Yet");
